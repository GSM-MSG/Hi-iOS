...
class SecondSignUpViewController: BaseViewController {
    ...
    private var can_send_certifycationNumber: Bool = false // textField 값이 이메일 형식과 같은 경우 이 멤버를 true 시켜준다. 인증번호 받기 버튼을 눌렀을 때 true 일 경우 인증번호 발송, false 일 경우 이메일의 형식이 다르다고 사용자에게 알려줌.
    ...
    private let cnUIButton = UIButton().then { // cirtification number 인증번호
    ...
    override func setLayout() {
        ...
        self.cnUIButton.snp.makeConstraints {
  ...
  @objc private func aftercomplete(_ sender: UIButton){
    ...
    @objc func cnBChecking(_ sender: UIButton){
        if can_send_certifycationNumber == true {
            nextStep = false
    ...
    @objc func textFieldDidChange(_ sender: Any?) {
        if can_send_certifycationNumber == false {
            emailCirtification()
        }
        else {
    ...
    func emailCirtification() {
        ...
        if id.count == 16{
            ...
            if mail == "@gsm.hs.kr"{
                can_send_certifycationNumber = true
                print("cnBCheck is true")
            }
            else {
                can_send_certifycationNumber = false
            }
        }
    }
}
